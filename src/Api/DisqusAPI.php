<?php namespace Mengidd\Disqus\Api;
/**
 ...
 * **********************************************************************
 *
 * Modifications by Fredrik Eriksen <fredrik@lamelol.com>
 *
 * * @link https://github.com/mengidd/Disqus-Laravel
 *
 * **********************************************************************
 */
...
$DISQUS_API_INTERFACES = json_decode(file_get_contents(dirname(__FILE__) . '/interfaces.json'));
...
class DisqusResource {
    ...
    public function __call($name, $args) {
        ...
        if (empty($kwargs['access_token']) && !is_null($api->access_token)) {
...
class DisqusAPI extends DisqusResource {
    ...
    public function __construct($key=null, $format='json', $version='3.0') {
        if ($key === null) {
            $this->key = Config::get('disqus.secret');
        } else {
            $this->key = $key;
        }
        $this->format = $format;
        $this->version = $version;
        $this->access_token = Config::get('disqus.access_token');
